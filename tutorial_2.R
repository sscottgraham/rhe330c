...
# This script is part of a tutorial on a tidy approach to data wrangling and includes some basic textstat functions. The code walks users through exploring and manipulating a dataframe imported from a .CSV with information about recent rhetoric courses at The University of Texas at Austin.   
...
# Which course descirptions are logest on average?
data %>% 
...
data %>% 
  mutate(read_diff = textstat_readability(Description, measure = "Flesch.Kincaid")) %>% 
  group_by(Course) %>% 
...
data %>% 
  ...
  unnest(cols = c(read_diff)) %>% 
  group_by(Course) %>% 
  summarise(mean_read_diff = mean(Flesch.Kincaid)) %>% 
  arrange(desc(mean_read_diff))

# Let's explore further 
data_diff <- data %>% 
  mutate(read_diff = textstat_readability(Description, measure = c("Flesch.Kincaid","Dale.Chall","SMOG"))) %>% 
  unnest(cols = c(read_diff))
...
data_diff %>% 
  group_by(Course) %>% 
  summarise(mean_fk = mean(Flesch.Kincaid), mean_dc = mean(Dale.Chall), mean_smog = mean(SMOG))
